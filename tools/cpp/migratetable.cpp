...
class agrss
{
  public:
    ...
    string tablename;     // 待处理的表名
    string desttablename; // 迁移的目标表名
    string keyfield;      // 待处理数据表的唯一字段名，如编号keyid
    string where;         // 待处理数据表的条件，如keyid>1000
    int maxcount;         // 待处理数据表的最大记录数，如1000
...
void migrateTable();
// 3.1获取表的全部字段
bool getField(shared_ptr<IConnection> connection, vector<string> &fieldNames);
// 3.2初始化插入sqlbase
void getInsertSql(string &res, const vector<string> &fieldNames);
...
void migrateTable()
{
    ...
    // 3.进行删除旧表/插入信标操作
    // 3.1 获取表的所有字段，由于两个表一致，所以只需要获取一个表的字段
    vector<string> fieldNames;
    if (!getField(connection, fieldNames))
    {
        lg.writeLine("获取表的字段失败");
        return;
    }
    // 初始化原始sql
    string deleteSqlbase = "delete from " + starg.tablename + " where " + starg.keyfield + " in (";
    string insertSqlbase;
    // 3.2初始化插入sql
    getInsertSql(insertSqlbase, fieldNames);
    // 得到结果：
    // insert into desttablename (field1,field2, field3) select field1,field2,field3 from orintablename where keyfield
    // in (

    // 2.获取待处理数据字段值的集合
    //**这段处理得放在getField函数之后，否则getField函数的查询结果会把这段处理给这里的内容给覆盖**
    ...
    string insertSql = insertSqlbase;
    // 读取数据
    int loopCount = 0;
    int totolCount = 0;
    while (connection->next())
    {
        ...
        if (loopCount >= starg.maxcount)
        {
            ...
            insertSql.back() = ')'; // 去掉最后一个逗号
            // 执行插入操作
            if (!connection->update(insertSql))
            {
                lg.writeLine("执行插入sql语句失败，sql语句：%s，报错代码:%d", insertSql.c_str(),
    ...
    if (loopCount > 0)
    {
        ...
        if (!connection->update(insertSql))
        {
            lg.writeLine("执行插入sql语句失败，sql语句：%s，报错代码:%d", insertSql.c_str(), connection->last_errno_);
            return;
        }
        if (!connection->update(deleteSql))
        {
    ...
    lg.writeLine("本次程序运行耗时：%.2f，共计处理记录%d条", runTime, totolCount);
}
bool getField(shared_ptr<IConnection> connection, vector<string> &fieldNames)
{
    // 组织获取表字段的sql
    string sql = "desc " + starg.tablename;
    // 执行sql
    ...
    {
        // 获取字段名
        optional<string> fieldName = connection->value(0);
        if (fieldName)
        {
            fieldNames.push_back(fieldName.value());
        }
    }
    return true;
}
...
void getInsertSql(string &res, const vector<string> &fieldNames)
{
    ...
    res = "insert into " + starg.desttablename + " (";
    // insert into desttablename (field1,field2,field3,
    ...
    // 去掉最后一个逗号
    res.back() = ')';
    // insert into desttablename (field1,field2, field3) select
    res += " select ";
    // insert into desttablename (field1,field2, field3) select field1,field2,field3,
    for (auto &it : fieldNames)
    {
        res += it + ",";
    }
    ...
    // insert into desttablename (field1,field2, field3) select field1,field2,field3 from orintablename where keyfield
    // in (
    res += " from " + starg.tablename + " where " + starg.keyfield + " in (";
}
...
bool parseMainParam(const string &jsonfile)
{
    ...
    if (!js.get("desttablename", starg.desttablename))
    {
        lg.writeLine("缺少desttablename参数");
        return false;
    }
...
void help()
{
    cout << "本程序是数据共享平台的通用模块，用于数据管理，把数据库表中的部分数据迁移到目标表" << endl;
    cout << "Usage: migratetable <logfile> <jsonfile>" << endl;
    cout << "logfile：用于保存数据入库日志的文件" << endl;
    cout << "jsonfile：用于保存程序传入参数的json文件" << endl;
    cout << "Example: ./migratetable /temp/log/migratetable.log "
            "/home/akijyo/桌面/code/c++/opendata/tools/others/migratetable.json"
         << endl
         << endl;
    ...
    cout << "tablename: 待处理数据库表名，不得为空。" << endl;
    cout << "desttablename: 迁移的目标表名" << endl;
    cout << "keyfield: 待处理数据库的唯一字段名，数据库的处理以此字段参考条件" << endl;
    cout << "where: 需要处理的数据被覆盖的条件" << endl;
